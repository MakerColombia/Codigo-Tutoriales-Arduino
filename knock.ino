/*
Sensor holpe, enciende el led conectado en el pin 13 al detectar golpe
Conecciones:
1- Pin OUT del sensor conectado al pin 7 del arduino.

Este sensor normalmente emite señal en alto, cuando detecta golpe pone la señal en bajo.
...
void loop () { 
  ...
  if (HIGH == valor){ // verifico si valor esta en alto 
    ...
    digitalWrite (ledPin, LOW); // detecto golpe, enciendo el led. 
  } 
  else { 
    digitalWrite (ledPin, HIGH); // no hay golpe, apago el led 
  } 
} 
